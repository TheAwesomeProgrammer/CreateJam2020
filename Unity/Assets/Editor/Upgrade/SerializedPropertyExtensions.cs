...
namespace Editor.Upgrade
{
    public static class SerializedPropertyExtensions
    {
        ...
        public static void SetStringArray(this SerializedProperty serializedProperty, string[] stringArray)
        {
            for (int i = 0; i < stringArray.Length; i++)
            {
                if (serializedProperty.arraySize <= i)
                {
                    serializedProperty.InsertArrayElementAtIndex(i);
                }
                SerializedProperty serializedPropertyAtIndex = serializedProperty.GetArrayElementAtIndex(i);
                serializedPropertyAtIndex.stringValue = stringArray[i];
            }
        }
    }
}